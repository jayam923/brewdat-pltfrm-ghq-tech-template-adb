...
synapse = "jdbc:sqlserver://brewdat-pltfrm-synwks-d.sql.azuresynapse.net:1433;database=poc_sqlpool;encrypt=true;trustServerCertificate=false;hostNameInCertificate=*.sql.azuresynapse.net;loginTimeout=30;Authentication=ActiveDirectoryIntegrated"
tempdir = "abfss://temp-csa@brewdatpltfrmsynwkssad.dfs.core.windows.net/dir_test"
df = spark.read \
  ...
  .option("dbTable", "dbo.ExternalUsers") \
  .load()
...
df1 = spark.read.format("csv").load( "abfss://temp-csa@brewdatpltfrmsynwkssad.dfs.core.windows.net/dir_test")
...
df1.write \
  ...
  .option("enableServicePrincipalAuth", "true") \
  .option("useAzureMSI", "true") \
  .option("dbTable", "dbo.ExternalUsers05") \
  .option("tempDir", "abfss://temp-csa@brewdatpltfrmsynwkssad.dfs.core.windows.net/dir_test") \
  .mode("append") \
  .save()
...
df1.write \
  .format("com.databricks.spark.sqldw") \
  .option("url", "jdbc:sqlserver://brewdat-pltfrm-synwks-d.sql.azuresynapse.net:1433;database=poc_sqlpool;encrypt=true;trustServerCertificate=false;hostNameInCertificate=*.sql.azuresynapse.net;loginTimeout=30;Authentication=ActiveDirectoryIntegrated") \
  .option("dbTable", "dbo.ExternalUsers00001") \
  .option("enableServicePrincipalAuth", "true") \
  .option("tempDir", "abfss://temp-csa@brewdatpltfrmsynwkssad.dfs.core.windows.net/dir_test") \
  .mode("append") \
  .save()
